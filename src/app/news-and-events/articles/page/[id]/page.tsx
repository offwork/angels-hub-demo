...
import HeroPost from "@/app/news-and-events/_components/AHHeroEvent";
...
import { getViewCategory, getViewCategoryHero } from "@/lib/fetch";
...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const eventHero = await getViewCategoryHero([category]);
  if (!eventHero) {
    return notFound();
  }
  const hero = eventHero.events[0];
  const viewAll = await getViewCategory(hero.id, PER_PAGE_SIZE, pageNumber, [category]);
  ...
  return {
    ...
    alternates: {
      canonical: `/news-and-events/${category}/${params.id}`,
    },
    title: `${meta.pages["news-and-events"].title} | Articles Page ${params.id}`,
    description: `${meta.pages["news-and-events"].description} Articles Page ${params.id}`,
  };
}

export default async function ArticlesPage({ params }: { params: { id: string } }) {
  if (isNaN(Number(params.id)) || Number(params.id) <= 0) return notFound();
