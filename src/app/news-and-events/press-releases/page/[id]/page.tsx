...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const eventHero = await getViewCategoryHero(["pressReleases"]);
  if (!eventHero) {
    return notFound();
  }
  const hero = eventHero.events[0];
  const viewAll = await getViewCategory(hero.id, PER_PAGE_SIZE, pageNumber, ["pressReleases"]);
  ...
  return {
    ...
    title: `${meta.pages["news-and-events"].title} | Press-Releases Page ${params.id}`,
    description: `${meta.pages["news-and-events"].description} Press-Releases Page ${params.id}`,
  };
}

export default async function PressReleasesPage({ params }: { params: { id: string } }) {
  if (isNaN(Number(params.id)) || Number(params.id) <= 0) return notFound();
